...
var poplayer = {
	...
	ahref:function(href,inputVal){
		if(!href){
			...
			if(inputVal){
				if(href.indexOf("?") != -1){
					window.location.href=href+'&pop_val2='+inputVal;
				}else{
	...
	html:function(text,btn1,btn2,inputflag){
		...
		pop_tip_html += '.pop_tip{position: fixed;top: 0;width: 720px;height: 100%;z-index: 999;background: rgba(0,0,0,0.5);}';
		pop_tip_html += '.pop_tip .pop_tip_box{position: fixed;top: 34%;width: 300px;left: 50%;margin-left:-150px;background: #fff;z-index: 1000;text-align: center;border-radius: 10px;}';
		...
		if(inputflag){
			pop_tip_html += '<div class="pop_tip_input_box" ><input type="text" name="tool" id="pop_tip_input" placeholder="'+inputflag+'"/></div>';
		}
		if(btn1 && btn2){
